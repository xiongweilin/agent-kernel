...
def _is_promotable(item: Any, evidence_by_id: dict[str, Any]) -> tuple[bool, str]:
    ...
    # Typed strict prerequisites (beyond evidence existence)
    ...
    # version context from environment_versions or metadata
    env_versions = (
    ...
    # At least one evidence must exist and be supported (still needed but not sufficient)
    has_supported = False
    for ref in ev_refs:
        ...
        if status == "supported":
            has_supported = True
            break
        # unverified/unknown no longer counts as promotable alone — must have explicit judgment
