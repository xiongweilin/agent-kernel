...
_IMPACT_ORDER: dict[str, int] = {"read": 0, "write-local": 1, "write-remote": 2, "deploy": 3, "admin": 4, "irreversible": 5}
...
def _is_side_effect_capability(contract, capability: str) -> bool:
    ...
    if lower.startswith("test.read") or lower.endswith(".read"):
        return False
    if "read" in lower and "deploy" not in lower and "write" not in lower:
        return False
    return True
