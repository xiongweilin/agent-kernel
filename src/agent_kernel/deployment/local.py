...
def create_local_runtime(state_path: Path, artifact_root: Path | None = None) -> Runtime:
    ...
    This is the one-click portable deployment used by:
        ...
        uv run python -m agent_kernel --state data/agent-kernel.db status
        uv run runtime --state data/agent-kernel.db work submit --title "Echo test" --kind generic-task
    ...
    Args:
        state_path: SQLite file for persistent state (e.g. ``Path("data/agent-kernel.db")``).
...
def create_personal_platform_runtime(state_path: Path, artifact_root: Path | None = None) -> Runtime:
    ...
    by the deployment entrypoint that loads ``control_plane.toml`` or ``agent_kernel.toml``.
