...
from agent_kernel.records.models import BaseRecord, EvidenceArtifact, OutcomeRecord
...
class InMemoryStateStore:
    ...
    def commit_terminal(self, work: Work, run: Run, verification_refs: list[str]) -> Run:
        ...
        with self.transaction():
            from agent_kernel.protocol.validation import assert_valid_terminal_pair
    ...
    def commit_verified_outcome(self, request: Any) -> OutcomeRecord:
        ...
        from agent_kernel.records.verified_outcome_commit import (
    ...
    def commit_recovery_disposition(self, request: Any, policy: Any) -> Any:
        ...
        from agent_kernel.workflows.recovery_disposition_commit import (
    ...
    def _validate_candidate_write(self, kind: str, value: BaseModel) -> None:
        ...
        from agent_kernel.protocol.validation import assert_valid_candidate_write
    ...
    def list_evidence(self, subject_ref: str | None = None) -> list[Evidence]:
        ...
        try:
            from agent_kernel.compat.legacy_records import evidence_artifact_to_legacy
    ...
    def get_knowledge(self, knowledge_id: str) -> KnowledgeItem | None:
        ...
        for projection in self._list("knowledge_projection", KnowledgeProjection):
            if projection.id == knowledge_id or projection.metadata.get("legacy_id") == knowledge_id:
                from agent_kernel.compat.legacy_records import knowledge_projection_to_legacy
    ...
    def append_event(self, value: Event) -> None:
        from agent_kernel.governance.outcome_impact_commit import OUTCOME_IMPACT_AUTHORITY_EVENT_TYPES
    ...
    def _validate_record_write_semantics(self, value: BaseRecord) -> None:
        from agent_kernel.protocol.validation import assert_semantic_mutation_authorized, validate_record_write
        from agent_kernel.records.validation import validate_canonical_write
    ...
    def save_relation(self, value: RecordRelation) -> None:
        from agent_kernel.records.relations import validate_relation
        with self._lock:
    ...
    def save_authorization(self, value: Any) -> None:
        ...
        with self._lock:
            if isinstance(value, AuthorizationGrant):
                from agent_kernel.records.authorization import validate_grant
    ...
    def save_authorization_use(self, value: Any) -> None:
        from agent_kernel.records.authorization import AuthorizationUse
        ...
        with self._lock:
            ...
            from agent_kernel.records.authorization import CanonicalAuthorizationRequest, is_authorized_for
    ...
    def import_state(self, state: dict[str, list[dict[str, object]]]) -> None:
        from agent_kernel.experience.historical_use import assert_historical_experience_use_import_closed
        ...
        try:
            from agent_kernel.records.authorization import AuthorizationGrant as _AuthGrant
            from agent_kernel.records.authorization import AuthorizationUse as _AuthUse
        ...
        with self._lock:
            ...
            from agent_kernel.protocol.validation import (
