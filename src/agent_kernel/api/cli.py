...
from agent_kernel.core.models import Event, Work, new_id
from agent_kernel.core.runtime import Runtime
from agent_kernel.plugin.conformance import check_provider
from agent_kernel.plugin.loader import load_manifest, validate_manifest
...
def run_cli(args: list[str]) -> int:
    ...
    parser.add_argument("--state", type=Path, default=Path("data/agent-kernel.db"))
    ...
    try:
        if parsed.command == "status":
            ...
            try:
                from agent_kernel.records.provenance import is_supported
                relations = runtime.store.list_relations()  # type: ignore
