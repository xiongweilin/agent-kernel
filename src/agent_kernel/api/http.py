...
def create_app(runtime: Runtime | None = None) -> FastAPI:
    ...
    async def metrics() -> Response:
        ...
        try:
            from agent_kernel.core.metrics import snapshot_run_status, snapshot_work_status
            ...
            for desc in runtime.registry.list():
                try:
                    ...
                    from agent_kernel.core.metrics import set_provider_health
    ...
    async def alertmanager_webhook(request: Request) -> dict[str, Any]:
        ...
        from agent_kernel.triggers.base import TriggerError as BTriggerError
    ...
    async def run_workflow(request: Request, work_id: str, workflow_id: str) -> dict[str, Any]:
        ...
        if workflow_id in {"incident-repair", "incident"}:
            ...
            from agent_kernel.workflows.daily_scan.workflow import DailyScanWorkflow
            workflow = DailyScanWorkflow()
        elif workflow_id == "knowledge-consolidation":
            from agent_kernel.workflows.knowledge_consolidation.workflow import KnowledgeConsolidationWorkflow
    ...
    async def reopen_record(request: Request, record_id: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        _require_local_control(request)
        from agent_kernel.records.reopen import ReopenAssessment, create_reopen_work
        payload = payload or {}
    ...
    async def explain_record(record_id: str) -> dict[str, Any]:
        from agent_kernel.records.provenance import lineage
        try:
            relations = runtime.store.list_relations()  # type: ignore
