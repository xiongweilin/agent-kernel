...
- fresh local proof: `uv run ruff check .`, `uv run mypy src`, `uv run pytest -q` → `261 passed`，strict-conformance selection → `63 passed`；此前基线提交已通过 [main CI run 32442371727](https://github.com/ratiolin/portable-runtime/actions/runs/32442371727)，本次四提交协议收敛序列将在推送后由新的 CI/SonarCloud 运行提供远端证据。
