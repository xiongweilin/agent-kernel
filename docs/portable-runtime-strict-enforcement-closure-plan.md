...
- fresh local proof: `uv run ruff check .`, `uv run mypy src`, `uv run pytest -q` → `244 passed`，strict-conformance selection → `61 passed`；本轮 Boundary 收敛、阈值配置去重与覆盖率历史测试清理已由 [main CI run 32447784428](https://github.com/ratiolin/portable-runtime/actions/runs/32447784428) 远端确认，SonarCloud new-code coverage 为 `80.1%` 且 quality gate 为 `OK`。
